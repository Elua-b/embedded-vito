...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        if 45 < Confidence < 85:
            ...
            # Check if the detected gesture is "OK" and update the database
            if gesture_class_name.strip() == "OK_Sign" and gesture_confidence_score > 0.9:
                try:
                    c.execute("UPDATE customers SET confirmed = 1 WHERE customer_uid = ?", (customer_uid,))
                    conn.commit()
                    print(f"Updated customer {customer_name}'s confirmation status to 1.")
                except sqlite3.Error as e:
                    print("SQLite error while updating:", e)
